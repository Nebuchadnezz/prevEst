...
The alternative to this is the assumption of no survival after the end of the follow up period. Under this condition, survival after the specified years of projected survival will go to 0. This analysis is run as follows, for projected survival to year 2030 with 18 years of survival data. 
...
<!-- Survival for years not in the survival dataset (e.g. for years there is not yet data) can be predicted using the convenient \code{survPredict} function. This function is simple a wrapper for the predict.lm method.  -->
...
Once prevalence and incidence are both properly formatted, using them in the \code{prevEst} function is simple. It is designed to take the cleaned inputs with little fuss on the researcher's part: simply state the year to calculate prevelance for. Further options are provided to expand output, if desired. In this case, using the previously cleaned data, incomplete prevalence is calculated for the year 2018.
...
Changing counts to rates per 100,000 using U.S. Census Population data for 2018
...
We can then visualize the single age histogram and the crude prevalence rates by 5 year age groups
...
Age-adjusted rates can be obtained manually, but, for convenience, this functionality is included in the \code{prevEst} function for users who specify \code{adjust = T}. This function can take grouped ages if need, as well. To use grouped ages, input a vector that you would like ages grouped by (i.e. seq(0,85, 5) if you want 5-year ages bins from 0 to 85, with 85+ grouped into 85). If no grouped ages are desired, the data can eb returned in single ages up to 85. Finally, if your data is sex-specific (e.g. as in the case of ovarian cancer), sex specific rates are also obtainable by specifying \code{sex_specific == "Male"} or \code{sex_specific == "Female"}.
...
We can compare crude versus age-adjusted prevalence rate
...
We can then compare the output of complete versus incomplete prevalence methods
